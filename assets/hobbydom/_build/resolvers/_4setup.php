...
switch (@$options[xPDOTransport::PACKAGE_ACTION]) {
	...
	case xPDOTransport::ACTION_UPGRADE:
		...
		$packages = array();
		if (!empty($options['install_packages']) && !empty($options['packages'])) {
			//$providers = $options['packages'];

			foreach ($options['install_packages'] as $package_name => $data) {
				$version = $options['packages'][$package_name]['versions'];
				...
				$prov = $options['packages'][$package_name]['prov'];
				$response = installPackage($package_name, $prov);
